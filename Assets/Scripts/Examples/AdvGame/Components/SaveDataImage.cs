...
namespace Assets.Scripts.Examples.AdvGame.Objects
{
    public class SaveDataImage : EGGameObject
    {
        ...
        // セーブデータの画像オブジェクトで、クリックでセーブ/ロードする
        ...
        private void Save()
        {
            var imageFilePath = $"{GameData.SAVE_DATA_FILE_PATH}/ScreenShots/image_{dataNumber}";
            var saveData = new SaveData(scenarioName, sceneId, messageNumber, imageFilePath);
        ...
        private void AddOnClickInLoadMenu()
        {
            if (saveData != null)
            {
                ...
                scenario.Load(savedata.SceneId, savedata.MessageNumber);
            }
        }
    }
}