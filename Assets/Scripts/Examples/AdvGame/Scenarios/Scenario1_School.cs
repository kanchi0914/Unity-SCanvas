...
namespace Assets.Scripts.Examples.AdvGame
{
    ...
    {
        public Scenario1_School() : base()
        {
            Renderer.SetBackgroundImage("bg_school_rouka");
        }
        ...
        protected override void InitScripts()
        {
            ...
            var script2 = new List<Section>()
            {
                ...
                new Section("ヒロシ", "いや　俺が主体になって動くのかよ…" +
                                   "\nおまえが言い出したくせに‥‥"),
                new Section("", "", () => { LoadScript("options_where_to_go"); }),
            };
            ...
            var script3 = new List<Section>()
            {
                ...
                new Section("ヒロ子", "‥‥‥‥‥‥。",
                    () => Renderer.SetCharacterImage("ヒロ子", "depressed")),
            ...
            var options_where_to_go = new List<Section>()
            {
                new Section("", "どこに行こうか？",
                    () =>
                    {
                        ...
                        var option1 = new Option($"{ScenarioName}_goto_saito", "斎藤さんの所",
                            () => LoadScript("goto_saito"));
                        var option2 = new Option($"{ScenarioName}_goto_class2", "2組の教室",
                            () => new Scenario2_Classroom().LoadScript());
                        var option3 = new Option($"{ScenarioName}_goto_pc_room", "パソコン室",
            ...
            {
                new Section("", "まずは斎藤さんの所へ行ってみた\nどうやらすごく落ち込んでいるようだ‥‥", () => { Renderer.RemoveAllCharacters(); }),
                new Section("ヒロシ", "あの‥‥斎藤さん\nちょっといいかな"),
                new Section("斎藤さん", "あ‥‥はい　なんでしょうか‥‥",
                    () => Renderer.SetCharacterImage("斎藤", "normal")),
            ...
            var options_what_to_ask = new List<Section>()
            {
                new Section("斎藤さん", "答えられること‥‥あまりないとおもいますが‥",
                    () =>
                    {
                        Renderer.RemoveAllCharacters();
                        Renderer.SetCharacterImage("斎藤", "normal");
                        var option1 = new Option($"{ScenarioName}_ask_about_wallet", "盗まれた財布について聞く",
                            () => LoadScript("ask_about_wallet"));
                        var option2 = new Option($"{ScenarioName}_ask_about_situation", "盗まれた時の状況について聞く",
                            () => LoadScript("ask_about_situation"));
                        var option3 = new Option(null, "やめる", () => { LoadScript("options_where_to_go"); });
                        Renderer.ShowOptionsWindow(option1, option2, option3);
                        // var optionWindow = new OptionsWindow(AdvMessageWindow);
                        ...
                        //     () => LoadScript("ask_about_situation")));
                        // optionWindow.AddOption("やめる", () => { LoadScript("options_where_to_go"); });
                    })
            };
            ...
            var ask_about_wallet = new List<Section>()
            {
                ...
                new Section("ヒロシ", "そうか‥‥\n(特に情報を引き出せそうにないな‥‥)"),
                new Section("", "", () => { LoadScript("options_what_to_ask"); }),
            };
            ...
            var solved_intro = new List<Section>()
            {
                ...
                new Section("ヒロ子", "え～気になるよ～",
                    () => Renderer.SetCharacterImage("ヒロ子", "smile")),
